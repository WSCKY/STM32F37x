...
/* -----------------------------------------------------------------------
	...
	TIM19CLK = PCLK2
	PCLK2 = HCLK
	=> TIM19CLK = HCLK = SystemCoreClock (72MHz or 64MHz)

	To get TIM19 counter clock at 8 MHz, the prescaler is computed as follows:
	Prescaler = (TIM19CLK / TIM1 counter clock) - 1
	Prescaler = ((SystemCoreClock) / 8 MHz) - 1 = 8 or 7

	To get TIM1 output clock at 8KHz, the period (ARR) is computed as follows:
...
/**
  ...
  *         @note :this value should be 0 ~ 1000.
  * @retval None
  */
...
{
	TIM_SetCompare1(TIM19, PWMRate1 * 10);
	TIM_SetCompare2(TIM19, PWMRate2 * 10);
	TIM_SetCompare3(TIM19, PWMRate3 * 10);
	TIM_SetCompare4(TIM19, PWMRate4 * 10);
}
