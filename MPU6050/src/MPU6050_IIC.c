...
void MPU6050_Correct(AcceDef *AcceOffset, GyroDef *GyroOffset)
{
	...
	do
	{
		...
		Turb =	ABS(DataGyr.X - OldDataGyr.X) + ABS(DataGyr.Y - OldDataGyr.Y) + ABS(DataGyr.Z - OldDataGyr.Z);
		if(Turb < 10)
			PeaceGyr ++;
		else
			PeaceGyr = 0;
...
//			PeaceAcc = 0;
		OldDataGyr = DataGyr;
//		OldDataGyr.X = DataGyr.X;
//		OldDataGyr.Y = DataGyr.Y;
//		OldDataGyr.Z = DataGyr.Z;
//		OldDataAcc.X = DataAcc.X;
//		OldDataAcc.Y = DataAcc.Y;
//		OldDataAcc.Z = DataAcc.Z;
		Delay_ms(5);
