/**
  ...
  * @version V0.0.0
  * @date    21-July-2015
  * @brief   Operate ubloxM6 moudle by uart in UBX mode.
  ******************************************************************************
  */
...
UBXFlag ubloxUBXProtoUARTConfig(UBXProtoUARTConfigDef* ConfigStructure)
{
	...
	ubloxSetUARTBaudRate(ConfigStructure->Baudrate);
	Index = 0;
	while(TestCnt)
	{
		if(DataOK == Set)
...
/**
  * @brief  Set Data rate.
  * @param  MeaFreq: Measurement Frequency.
  * @param  NavRate: Navigation Rate.
  * @param  TimeRef: Alignment to reference time.
  *         @note  This Value must be: TimeRef_UTC or TimeRef_GPS
  * @retval None
  */
void ubloxSetRate(uint8_t MeaFreq, uint8_t NavRate, uint8_t TimeRef)
{
	uint8_t Index  = 0;
	uint16_t MeaRate = 1000 / MeaFreq;
	for(Index = 0; Index < 6; Index ++)
		PrivateBuffer[Index] = 0x0;
	PrivateBuffer[0] = MeaRate & 0xFF;
	PrivateBuffer[1] = (MeaRate >> 8) & 0xFF;
	PrivateBuffer[2] = NavRate & 0xFF;
	PrivateBuffer[3] = (NavRate >> 8) & 0xFF;
	PrivateBuffer[4] = TimeRef;
	UBXStructure.SYNCHAR1 = UBX_SYNCHAR1;
	UBXStructure.SYNCHAR2 = UBX_SYNCHAR2;
	UBXStructure.CLASS    = UBX_CLASS_CFG;
	UBXStructure.ID       = UBX_CFG_RATE;
	UBXStructure.LENGTH   = 6;
	UBXStructure.pPayload = PrivateBuffer;
	CompChkSumVal(&UBXStructure);
	SendUBXStruct(&UBXStructure);

	UBXStructure.LENGTH   = 0;
	CompChkSumVal(&UBXStructure);
	SendUBXStruct(&UBXStructure);
}
