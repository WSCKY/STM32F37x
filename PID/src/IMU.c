...
//	Norm = sqrt(Acc->X * Acc->X + Acc->Y * Acc->Y + Acc->Z * Acc->Z);
	...
	q0 = (1-Theta_2/8)*q0 + (-q1*Gyr->X - q2*Gyr->Y - q3*Gyr->Z)*halfT;
	q1 = (1-Theta_2/8)*q1 + (q0*Gyr->X + q2*Gyr->Z - q3*Gyr->Y)*halfT;
	q2 = (1-Theta_2/8)*q2 + (q0*Gyr->Y - q1*Gyr->Z + q3*Gyr->X)*halfT;
	q3 = (1-Theta_2/8)*q3 + (q0*Gyr->Z + q1*Gyr->Y - q2*Gyr->X)*halfT;
...
//	Q->q0 = q0; Q->q1 = q1; Q->q2 = q2; Q->q3 = q3;
	...
	Angle->Pitch = asin(-2 * q1 * q3 + 2 * q0 * q2)* 57.3 - Pitch_Offset;
	Angle->Roll = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1) * 57.3 - Roll_Offset;
	Angle->Yaw = atan2(2 * q1 * q2 + 2 * q0 * q3, -2 * q2 * q2 - 2 * q3 * q3 + 1) * 57.3 - Yaw_Offset;
}
